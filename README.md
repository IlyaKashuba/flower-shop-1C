...
- **Бухгалтерии** `Управленческий`- бухгалтерский учет по счетам 41 (Товары), 90 (Капитал), 60 (Расчеты с поставщиками), 62 (Дебиторская задолженность) 
...
- `Оборотно-сальдовая ведомость` - отчет по счетам 41, 60, 62, 90
...
-  **Продажа**:
  ...
  - Расчёт суммы продажи: `Цена` (из регистра `Цены`) x `Количество` - `Скидка клиенту` (из справочника `Клиенты`)
